...
namespace dg_bolt
{
  ...
  void DGMBolt::initialize_hardware_communication_process()
  {
    ...
    ros_user_commands_.push_back(ros_node_handle.advertiseService(
        ...
        &DGMBolt::calibrate_joint_position_callback, this));
  ...
  void DGMBolt::get_sensors_to_map(dynamic_graph::VectorDGMap& map)
  {
    ...
    const std::array<bool, BOLT_NB_MOTOR>& motor_enabled = bolt_.get_motor_enabled();
    const std::array<bool, BOLT_NB_MOTOR>& motor_ready = bolt_.get_motor_ready();
    const std::array<bool, BOLT_NB_MOTOR_BOARD>& motor_board_enabled = bolt_.get_motor_board_enabled();
    const std::array<int, BOLT_NB_MOTOR_BOARD>& motor_board_errors = bolt_.get_motor_board_errors();
      ...
      const dynamic_graph::VectorDGMap& map)
  {
    try{
      ...
      rt_printf("DGMBolt::set_motor_controls_from_map: "
